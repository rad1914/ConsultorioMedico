    ...
    public partial class frmRecetas : Form
    {
        ...
        private void cboPaciente_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand(@"
                SELECT TOP 1 c.IdCita, c.IdMedico, e.IdExpediente
                FROM Citas c
        ...
        private void cmdRegistrar_Click_1(object sender, EventArgs e)
        {
            ...
            SqlCommand cmdReceta = new SqlCommand(@"
                IF NOT EXISTS (SELECT 1 FROM Receta WHERE IdExpediente = @idExp)
                BEGIN
                    INSERT INTO Receta (IdExpediente, IdMedico)
                    VALUES (@idExp, @idMedico);
                END

                SELECT IdReceta FROM Receta WHERE IdExpediente = @idExp;", conn);
