...
namespace ConsultorioMedico
{
    ...
    {
        SqlConnection conn = new SqlConnection("Server=(LocalDb)\\MSSQLLocalDB;Initial Catalog=Sistema;Integrated Security=True;TrustServerCertificate=True;");
        ...
        private void cmdBuscar_Click(object sender, EventArgs e)
        {
            ...
            SELECT C.IdCita, C.Fecha, P.IdPaciente, 
                    ...
                    P.Genero, P.FechaNac, P.TipoSangre,
                    P.Alergias, P.EnfermedadCronica,
                    C.Hora, C.Estado AS Edo
            FROM Citas C
            JOIN Pacientes P ON C.IdPaciente = P.IdPaciente
            WHERE C.Fecha = @fecha AND C.Estado = 'R'";

            adapterCitas = new SqlDataAdapter(query, conn);
            adapterCitas.SelectCommand.Parameters.Add("@fecha", SqlDbType.Date).Value = dtpFecha.Value.Date;
        ...
        private void cmdRegistrar_Click(object sender, EventArgs e)
        {
            ...
            cmd.Parameters.AddWithValue("@idCita", (int)cboCitas.SelectedValue);
            ...
            cmd.Parameters.Add("@peso", SqlDbType.Decimal).Value = decimal.Parse(txtPeso.Text);
            cmd.Parameters.Add("@est", SqlDbType.Decimal).Value = decimal.Parse(txtEstatura.Text);
            cmd.Parameters.Add("@temp", SqlDbType.Decimal).Value = decimal.Parse(txtTemperatura.Text);

            conn.Open();
            cmd.ExecuteNonQuery();

            SqlCommand cmdUpdate = new SqlCommand("UPDATE Citas SET Estado = 'A' WHERE IdCita = @idCita", conn);
            cmdUpdate.Parameters.AddWithValue("@idCita", (int)cboCitas.SelectedValue);
